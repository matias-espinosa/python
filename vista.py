...
class Ventana:
    def __init__(self, window) -> None:
        ...
        self.boton_cerrar=Button(self.root, text="Cerrar Aplicación", width=button_width, command=lambda:cerrar_programa(root, self.tree))
        self.boton_cerrar.grid(row=0, column=3, sticky=E, padx=10)

        self.boton_alta=Button(text="Agregar tiempo", width=button_width, command=lambda:alta_vista())
        self.boton_alta.grid(row=6, column=3, padx=10, pady=5, sticky=E)

        self.boton_limpiar=Button(text="Limpiar", width=6, command=lambda:limpiar(self.dni_value, self.nombre_value, self.tiempo_value, self.entry_dni, self.tree))
        self.boton_limpiar.grid(row=6, column=2,sticky=E)

        self.boton_modificar=Button(button_frame_top, text="Modificar tiempo", width=button_width, command=lambda:modificar_vista())
        self.boton_modificar.grid(row=9, column=2, padx=12)

        self.boton_borrar=Button(button_frame_top, text="Borrar tiempo", width=button_width, command=lambda:self.objeto_interaccionBd.borrar(self.tree))
        self.boton_borrar.grid(row=9, column=1, padx=12)

        self.boton_mejor_tiempo=Button(button_frame_bottom, text="Mejor tiempo", width=button_width, command=lambda:self.objeto_interaccionBd.mejor_tiempo(self.tree))
        self.boton_mejor_tiempo.grid(row=13, column=2, padx=12)

        self.boton_consulta=Button(button_frame_bottom, text="Listar tiempos", width=button_width, command=lambda:actualizar_treeview(self.tree)) 
